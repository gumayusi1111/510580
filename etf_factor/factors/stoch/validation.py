...
class StochValidation:
    ...
    def validate_output_data(result: pd.DataFrame, params: dict) -> tuple[bool, str]:
        ...
        expected_columns = ['ts_code', 'trade_date', f'STOCH_K_{k_period}', f'STOCH_D_{d_period}']
    ...
    def validate_calculation_consistency(data: pd.DataFrame, result: pd.DataFrame, params: dict) -> tuple[bool, str]:
        ...
        if len(data) >= k_period:
            ...
            if manual_high != manual_low:
                ...
                k_col = f'STOCH_K_{k_period}'
                calculated_k = result[k_col].iloc[-1]

                # 允许小的数值误差（0.01）
                if abs(calculated_k - manual_k) > 0.01:
                    return False, f"计算不一致: 手工%K={manual_k:.4f} vs 计算%K={calculated_k:.4f}"
