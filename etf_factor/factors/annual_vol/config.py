...
class AnnualVolConfig:
    ...
    def _validate_periods(cls, periods) -> list:
        ...
        for period in periods:
            ...
            if period < cls.MIN_PERIOD:
                raise ValueError(f"周期不能小于{cls.MIN_PERIOD}(至少需要2个数据点计算标准差): {period}")
    ...
    def get_factor_info(cls, params: dict) -> dict:
        ...
        return {
            ...
            'formula': 'ANNUAL_VOL = STD(日收益率) × √252',
            'output_columns': [f'ANNUAL_VOL_{p}' for p in params['periods']]
        }

    @classmethod
    def get_expected_output_columns(cls, params: dict) -> list:
        """获取预期输出列"""
        return [f'ANNUAL_VOL_{p}' for p in params['periods']]

    @classmethod
    def get_trading_days_per_year(cls) -> int:
        """获取年化交易日数"""
        return cls.TRADING_DAYS_PER_YEAR