...
class WrConfig:
    ...
    def get_factor_info(cls, params: dict) -> dict:
        return {
            'name': 'WR',
            'description': '威廉指标 - 衡量超买超卖的动量指标',
            'category': 'volume_price',
            'period': params['period'],
            'data_type': 'momentum_oscillator',
            'calculation_method': 'williams_percent_r',
            'formula': 'WR = (HHV - CLOSE) / (HHV - LLV) × (-100)',
            'output_columns': [f'WR_{params["period"]}']
        }

    @classmethod
    def get_expected_output_columns(cls, params: dict) -> list:
        return [f'WR_{params["period"]}']